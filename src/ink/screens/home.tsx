...
export function HomeScreen({ onSelect, pinnedDomain, onClearPinnedDomain }: HomeScreenProps) {
  ...
  // only a second Escape (now with nothing pinned) actually quits.
  const { bindings } = useKeymap([
    ...
    { key: 'escape', label: pinnedDomain ? 'forget domain' : 'quit', onTrigger: () => (pinnedDomain ? onClearPinnedDomain() : process.exit(0)) },
  ])
